...
--- Getters
-- Takes in a vertex, a list of vertexes, and returns true if the  vertex is in the list.
...
checkVertexs :: [Vertex] -> [Vertex] -> Bool
checkVertexs [] _ = True
checkVertexs g []  = True
...
updateDistPredbylabel::[Vertex] -> String -> String -> Int -> [Vertex]
...
bfs_helper inGraph queue seen outGraph =  bfs_helper inGraph new_queue new_seen new_outgraph
   where 
     -- get the vertex from the queue 
       (newqueue,(Vertex label neighbors distance _)) = popQueue queue

       -- get the labels of the neighbors  of the vertex 
       neighborslabels =  map( \(vlabel,_) -> vlabel) neighbors

      
       -- filter out vertices  that were seen already labels
       ...
       -- update the distance and predecessor for those  vertices 
...
--- Take the output of a BFS and return  the path for the source of the BFS to a given vertex 
...
neighborscontains [] _ = False
neighborscontains ((nlabel,_):xs) l = l == nlabel || neighborscontains xs l 
...
minCostforPath g [_] = (maxBound::Int)
minCostforPath g (x:y:xs)  = min xyCost restCost where 
                                                     (Vertex _ n _ _ ) = head $ filter (\ (Vertex label' _ _ _ ) -> label' == x) g
                                                     (_ ,xyCost) = head $ filter (\ (label' ,_) -> label' == y) n
...
reduceCost' [] _ _ = []  
reduceCost' ((label,oldcost):_) dest  amount  | label == dest && oldcost - amount < 0 = error "invalid cost update"
...
reduceEdgeCost g src dest reduceAmount   = newGraph  where 
                                              updatenewGraph = map (\(Vertex label' n' p' d' ) -> if label' == src then  (Vertex label' (reduceCost n' dest reduceAmount) p' d' ) else (Vertex label' n' p' d' ) ) g
